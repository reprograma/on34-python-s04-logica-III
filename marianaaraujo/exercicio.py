...
def quantidade_palavra(texto, palavra):
    texto_dividido = texto.split(" ")
    quantidade = 0
    for lei in texto_dividido:
        if lei == palavra:
            quantidade += 1
    return quantidade
...
def main ():
    ...
    texto_escolhido = []
    indice = 0
    for contador in textos:
       indice = indice+1
       texto_escolhido.append(quantidade_palavra(contador, 'lei'))
    maior_numero = max(texto_escolhido)
    print ('O texto que contém mais a palavra lei é o:\n',textos[texto_escolhido.index(maior_numero)])
