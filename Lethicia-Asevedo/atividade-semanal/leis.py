...
# Função para contar ocorrências da palavra "lei"
def contar_leis(texto):
    palavras = texto.lower().split()  
    return palavras.count('lei')
...
for i, texto in enumerate(textos):
    contagens[f'texto{i+1}'] = contar_leis(texto)


texto_escolhido = max(contagens, key=contagens.get)


print(f'O texto escolhido foi o {texto_escolhido}.')

print("Contagens das ocorrências de 'lei':")
for texto, contagem in contagens.items():
    print(f'{texto}: {contagem}')
